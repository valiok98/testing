...
static void on_trackbar(int pos, void* slider_value) {
	*((int*)slider_value) = pos;
	...
	// E.g. all values from 170 to 255 are black, the rest under 170 is white. Thresholding take the image as a matrix.
...
int main() {
	...
	while (cap.read(frame)) {
		...
		//drawContours(result_frame, contours, -1, Scalar(0, 255, 0), 4, 1);
		...
		for (size_t k = 0; k < contours.size(); k++) {
			...
			// Simplifying of the contour with the Ramer-Douglas-Peuker
			// arcLength = perimeter (Umfang) -> Every contour bigger than that will be accepted, smallers ignored, true -> closed contour
			// Approximation of old curve, the difference (epsilon) should not be bigger than: permeter*0.02
